...
interface Metric { timestamp: string; latency_ms: number; throughput_rpm: number; error_rate: number; quality_score: number; drift_score: number; availability: number; }
...
<section class="layout"><div class="inventory"><h2>Inventory</h2><ng-container *ngIf="models$ | async as models; else loading"><article class="model" *ngFor="let model of models" (click)="select(model)" [class.selected]="selected?.model_id === model.model_id"><div><strong>{{ model.name }}</strong><small>{{ model.model_id }} · {{ model.framework }}</small></div><span>{{ model.versions.length }} versions</span></article><p class="empty" *ngIf="!models.length">No models match this search.</p></ng-container><ng-template #loading><p class="empty">Loading model inventory...</p></ng-template></div>
<div class="detail" *ngIf="selected as model; else choose"><div class="detail-head"><div><p class="eyebrow">SELECTED MODEL</p><h2>{{ model.name }}</h2><p>{{ model.owner }} · {{ model.framework }}</p></div><button (click)="loadMetrics(model.model_id)">Refresh metrics</button></div><h3>Versions</h3><div class="versions"><div class="version" *ngFor="let item of model.versions"><div><strong>v{{ item.version }}</strong><span class="pill" [class.production]="item.stage === 'PRODUCTION'">{{ item.stage }}</span></div><small>{{ item.approved ? 'Approved for release' : 'Awaiting approval' }}</small><div class="version-actions"><button *ngIf="canTransition(item, 'VALIDATED')" (click)="changeLifecycle(item, 'VALIDATED')">Validate</button><button *ngIf="canTransition(item, 'APPROVED')" (click)="changeLifecycle(item, 'APPROVED')">Approve</button><button *ngIf="canTransition(item, 'STAGING')" (click)="changeLifecycle(item, 'STAGING')">Promote to staging</button><button *ngIf="canTransition(item, 'PRODUCTION')" (click)="changeLifecycle(item, 'PRODUCTION')">Promote to production</button></div></div></div><h3>Monitoring snapshot</h3><ng-container *ngIf="metrics$ | async as metrics; else metricsLoading"><div class="metrics" *ngIf="metrics.length; else noMetrics"><div><b>{{ metrics[0].latency_ms | number:'1.0-0' }} ms</b><small>Latency</small></div><div><b>{{ metrics[0].throughput_rpm | number:'1.0-0' }}</b><small>Throughput / min</small></div><div><b>{{ metrics[0].quality_score | percent }}</b><small>Quality</small></div><div><b>{{ metrics[0].availability | number:'1.2-2' }}%</b><small>Availability</small></div></div><ng-template #noMetrics><p class="empty">No monitoring data for this model.</p></ng-template></ng-container><ng-template #metricsLoading><p class="empty">Select refresh to load metrics.</p></ng-template></div><ng-template #choose><div class="detail empty"><h2>Select a model</h2><p>Choose an inventory item to inspect versions and health.</p></div></ng-template></section>
...
export class AppComponent {
  ...
  models$: Observable<Model[]> = this.http.get<Model[]>(`${this.api}/models`).pipe(catchError(() => of([])));
  metrics$: Observable<Metric[]> = of([]); deployments$: Observable<Deployment[]> = of([]); selected?: Model; selectedDeployment?: Deployment;
  ...
  constructor() { this.loadDeployments(); }
  load(search = ''): void { this.models$ = this.http.get<Model[]>(`${this.api}/models?search=${encodeURIComponent(search)}`).pipe(catchError(() => of([]))); }
  select(model: Model): void { this.selected = model; this.deploymentForm.model_id = model.model_id; this.loadMetrics(model.model_id); this.loadDeployments(model.model_id); }
  loadMetrics(modelId: string): void { this.metrics$ = this.http.get<Metric[]>(`${this.api}/models/${modelId}/metrics`).pipe(catchError(() => of([]))); }
